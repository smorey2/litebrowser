...
void CHTMLViewWnd::OnPaint(simpledib::dib* dib, LPRECT rcDraw)
{
	...
	if (page)
	{
		...
		page->m_doc->draw((litehtml::uint_ptr)cr, -m_left, -m_top, 0, &clip);
...
void CHTMLViewWnd::OnMouseMove(int x, int y)
{
	...
	if (page)
	{
		...
		if (page->m_doc->on_mouse_over(x + m_left, y + m_top, 0, x, y, 0, redraw_boxes))
		{
...
void CHTMLViewWnd::OnLButtonDown(int x, int y)
{
	...
	{
		litehtml::position::vector redraw_boxes;
		if (page->m_doc->on_lbutton_down(x + m_left, y + m_top, 0, x, y, 0, redraw_boxes))
		{
			for (litehtml::position::vector::iterator box = redraw_boxes.begin(); box != redraw_boxes.end(); box++)
			{
				box->x -= m_left;
...
void CHTMLViewWnd::OnLButtonUp(int x, int y)
{
	...
	{
		litehtml::position::vector redraw_boxes;
		if (page->m_doc->on_lbutton_up(x + m_left, y + m_top, 0, x, y, 0, redraw_boxes))
		{
			for (litehtml::position::vector::iterator box = redraw_boxes.begin(); box != redraw_boxes.end(); box++)
			{
				box->x -= m_left;
