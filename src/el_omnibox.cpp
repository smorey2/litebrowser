...
void el_omnibox::draw(litehtml::uint_ptr hdc, int x, int y, int z, const litehtml::position* clip)
{
	litehtml::html_tag::draw(hdc, x, y, z, clip);
...
BOOL el_omnibox::OnLButtonDown(int x, int y)
{
	...
	{
		litehtml::position pos = litehtml::element::get_placement();
		if (m_edit.in_capture() || pos.is_point_inside(x, y, 0))
		{
